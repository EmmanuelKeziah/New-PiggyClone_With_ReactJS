// import React, { useState } from "react";
// import { MdKeyboardArrowDown } from "react-icons/md";
// import { Link } from "react-router-dom";
// import Nav from "./Nav";
// // import MenuItem from "../components/reusable/MenuItem";

// const NavHover=() => {
//   const [save, setSave] = useState(false);
//   const [resource, setResource] = useState(false);

//   return (
//     <div className="w-full">
//       {/* Desktop Navbar */}
//       <div className="w-full flex justify-between items-center max-w-[1200px] mx-auto py-5 max-md:hidden">
//         {/* Logo and Links */}
//         <div className="w-[70%] flex justify-between items-center">
//           <div className="w-[20%]">
//             <img
//               src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg"
//               alt="Logo"
//               className="bg-transparent"
//             />
//           </div>

//           </div>
//           <div className="flex justify-between items-center w-[70%] gap-x-[10px] font-[600] text-[16px] text-[#3d4f60]">
//             <div className="">
//               <span
//                 onMouseEnter={() => {
//                   setSave(true);
//                 }}
//                 onMouseLeave={() => {
//                   setSave(false);
//                 }}
//                 className="flex items-center gap-x-[5px]"
//               >
//                 Save <MdKeyboardArrowDown />
//               </span>

//               <div
//                 onMouseEnter={() => {
//                   setSave(true);
//                 }}
//                 onMouseLeave={() => {
//                   setSave(false);
//                 }}
//                 className={
//                   !save
//                     ? "hidden"
//                     : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
//                 }
//               >
//                 <div className="w-[50%]  flex flex-col gap-y-[30px] ">
//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/piggybank_icon_bd14f39497_7ef36812cd/piggybank_icon_bd14f39497_7ef36812cd.png"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">PiggyBank</p>
//                       <p className="font-[600] text-[14px]">
//                         Automated Savings
//                       </p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/targets_icon_a382a2df4f_10353cd31e/targets_icon_a382a2df4f_10353cd31e.png"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Target Savings</p>
//                       <p className="font-[600] text-[14px]">
//                         Goal-Oriented Savings
//                       </p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_dollar_icon_b180813700_8c5227fe52/flex_dollar_icon_b180813700_8c5227fe52.png"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Flex Dollar</p>
//                       <p className="font-[600] text-[14px]">Dollar Savings</p>
//                     </div>
//                   </div>
//                 </div>

//                 {/* RIGHT SIDE */}

//                 <div className="w-[50%]  flex flex-col gap-y-[30px]">
//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/safelock_icon_7595628576_b010a4fee8/safelock_icon_7595628576_b010a4fee8.png"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Safelock</p>
//                       <p className="font-[600] text-[14px]">Fixed Savings</p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_naira_icon_1c37272848_635abfe249/flex_naira_icon_1c37272848_635abfe249.png"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Flex Naira</p>
//                       <p className="font-[600] text-[14px]">Flexible Savings</p>
//                     </div>
//                   </div>

//                   <Link to={"/housemoney"}>
//                     <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                       <div className=" w-[35px] ">
//                         <img
//                           className="w-[35px] h-[35px]"
//                           src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/house_icon_1_62b781ec0c/house_icon_1_62b781ec0c.png"
//                           alt=""
//                         />
//                       </div>
//                       <div className=" w-[70%]">
//                         <p className="font-[600] text-[16px]">House Money</p>
//                         <p className="font-[600] text-[14px]">
//                           Semi-Strict Wallet
//                         </p>
//                       </div>
//                     </div>
//                   </Link>
//                 </div>
//               </div>
//             </div>

//             <Link to={"/invest"}>
//               <span>Invest</span>
//             </Link>

//             <span>Stories</span>
//             <span>FAQ</span>

//             <div className="">
//               <span
//                 onMouseEnter={() => {
//                   setResource(true);
//                 }}
//                 onMouseLeave={() => {
//                   setResource(false);
//                 }}
//                 className="flex items-center gap-x-[5px]"
//               >
//                 Resources <MdKeyboardArrowDown />
//               </span>

//               <div
//                 onMouseEnter={() => {
//                   setResource(true);
//                 }}
//                 onMouseLeave={() => {
//                   setResource(false);
//                 }}
//                 className={
//                   !resource
//                     ? "hidden"
//                     : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
//                 }
//               >
//                 <div className="w-[50%]  flex flex-col gap-y-[30px]">
//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Blog</p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Comics</p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">NewsLetter</p>
//                     </div>
//                   </div>
//                 </div>

//                 {/* RIGHT SIDE */}

//                 <div className="w-[50%]  flex flex-col gap-y-[30px]">
//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Reports</p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Calculatoe</p>
//                     </div>
//                   </div>

//                   <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
//                     <div className=" w-[35px] ">
//                       <img
//                         className="w-[35px] h-[35px]"
//                         src="data:image/svg+xml;base64,"
//                         alt=""
//                       />
//                     </div>
//                     <div className=" w-[70%]">
//                       <p className="font-[600] text-[16px]">Ebook</p>
//                     </div>
//                   </div>
//                 </div>
//               </div>
//             </div>

//             <Link to={"/shop"}>
//               <span>Shop</span>
//             </Link>
//           </div>

//           {/* <div className="flex justify-between items-center w-[70%] gap-2 text-[#3d4f60] font-semibold text-[16px]"> */}
//             {/* Save Dropdown */}
//             <div
//               onMouseEnter={() => setSave(true)}
//               onMouseLeave={() => setSave(false)}
//               className="relative"
//             >
//               <span className="flex items-center gap-1 cursor-pointer">
//                 Save <MdKeyboardArrowDown />
//               </span>

//               {/* {save && (
//                 <div className="absolute z-20 top-8 w-[500px] bg-white shadow-md rounded-xl p-5 flex gap-5">
//                   <div className="w-1/2 flex flex-col gap-6">
//                     {[
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/piggybank_icon_bd14f39497_7ef36812cd/piggybank_icon_bd14f39497_7ef36812cd.png",
//                         title: "PiggyBank",
//                         desc: "Automated Savings",
//                       },
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/targets_icon_a382a2df4f_10353cd31e/targets_icon_a382a2df4f_10353cd31e.png",
//                         title: "Target Savings",
//                         desc: "Goal-Oriented Savings",
//                       },
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_dollar_icon_b180813700_8c5227fe52/flex_dollar_icon_b180813700_8c5227fe52.png",
//                         title: "Flex Dollar",
//                         desc: "Dollar Savings",
//                       },
//                     ].map((item, i) => (
//                       <div key={i} className="flex items-center gap-3">
//                         <img
//                           src={item.img}
//                           alt={item.title}
//                           className="w-9 h-9"
//                         />
//                         <div>
//                           <p className="text-[16px] font-semibold">
//                             {item.title}
//                           </p>
//                           <p className="text-[14px] font-semibold text-gray-500">
//                             {item.desc}
//                           </p>
//                         </div>
//                       </div>
//                     ))}
//                   </div>

//                   <div className="w-1/2 flex flex-col gap-6">
//                     {[
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/safelock_icon_7595628576_b010a4fee8/safelock_icon_7595628576_b010a4fee8.png",
//                         title: "Safelock",
//                         desc: "Fixed Savings",
//                       },
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_naira_icon_1c37272848_635abfe249/flex_naira_icon_1c37272848_635abfe249.png",
//                         title: "Flex Naira",
//                         desc: "Flexible Savings",
//                       },
//                       {
//                         img: "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/house_icon_1_62b781ec0c/house_icon_1_62b781ec0c.png",
//                         title: "House Money",
//                         desc: "Semi-Strict Wallet",
//                         to: "/housemoney",
//                       },
//                     ].map((item, i) => {
//                       const content = (
//                         <div key={i} className="flex items-center gap-3">
//                           <img
//                             src={item.img}
//                             alt={item.title}
//                             className="w-9 h-9"
//                           />
//                           <div>
//                             <p className="text-[16px] font-semibold">
//                               {item.title}
//                             </p>
//                             <p className="text-[14px] font-semibold text-gray-500">
//                               {item.desc}
//                             </p>
//                           </div>
//                         </div>
//                       );

//                       return item.to ? (
//                         <Link to={item.to} key={i}>
//                           {content}
//                         </Link>
//                       ) : (
//                         content
//                       );
//                     })}
//                   </div>
//                 </div>
//               )} */}
//             </div>

//             <Link to="/invest">
//               <span className="cursor-pointer">Invest</span>
//             </Link>

//             <span className="cursor-pointer">Stories</span>
//             <span className="cursor-pointer">FAQ</span>

//             {/* Resources Dropdown */}
//             <div
//               onMouseEnter={() => setResource(true)}
//               onMouseLeave={() => setResource(false)}
//               className="relative"
//             >
//               <span className="flex items-center gap-1 cursor-pointer">
//                 Resources <MdKeyboardArrowDown />
//               </span>

//               {resource && (
//                 <div className="absolute z-20 top-8 w-[300px] bg-white shadow-md rounded-xl p-5 flex flex-col gap-4">
//                   {["Blog", "Comics", "Newsletter", "Reports"].map(
//                     (item, i) => (
//                       <span
//                         key={i}
//                         className="text-[16px] font-medium text-gray-700 cursor-pointer"
//                       >
//                         {item}
//                       </span>
//                     )
//                   )}
//                 </div>
//               )}
//             </div>
//           </div>
//         </div>

//         {/* Login and Create Account */}
//         <div className="flex items-center gap-6">
//           <button className="text-[#122231] text-[15px] font-semibold">
//             Sign in
//           </button>
//           <button className="bg-[#0c1825] px-5 py-2 rounded-md text-white text-[15px] font-semibold">
//             Create free account
//           </button>
//         </div>
//       </div>

//       {/* Mobile Nav (using Nav) */}
//       <div className="md:hidden">
//         <Nav />
//       </div>
//     </div>
//   );
// }

// import React from "react";
// import { useState } from "react";
// import { MdKeyboardArrowDown } from "react-icons/md";
// import Nav from "./Nav";
// import { Link } from "react-router-dom";
// import MenuItem from "../components/reusable/MenuItem";

// const NavHover = () => {
//   const [save, setSave] = useState(false);
//   const [resource, setResource] = useState(false);

//   return (
//     <div className="w-[100%]">
//       <div className="w-ful flex justify-between items-center max-w-[1200px] mx-auto py-5 max-md:hidden ">
//         <div className="w-[70%] flex justify-between items-center">
//           <div className="w-[20%]">
//             <img
//               className="bg-transparent"
//               src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg"
//               alt="Logo"
//             />
//           </div>
//           <div className="flex justify-between items-center w-[70%] gap-2 font-semibold text-[16px] text-[#3d4f60]">
//             <div
//               onMouseEnter={() => setSave(true)}
//               onMouseLeave={() => setSave(false)}
//               className="relative"
//             >
//               <span className="flex items-center gap-1 cursor-pointer">
//                 Save <MdKeyboardArrowDown />
//               </span>

//               {/* {{save && (
//                 <div className="absolute z-20 top-8 w-[500px] bg-white shadow-md rounded-xl p-5 flex gap-5">
//               )}} */}

//               {/* <span
//                 onMouseEnter={() => {
//                   setSave(true);
//                 }}
//                 onMouseLeave={() => {
//                   setSave(false);
//                 }}
//                 className="flex items-center gap-x-[5px]"
//               >
//                 Save <MdKeyboardArrowDown />
//               </span> */}

//               <div
//                 onMouseEnter={() => {
//                   setSave(true);
//                 }}
//                 onMouseLeave={() => {
//                   setSave(false);
//                 }}
//                 className={
//                   !save
//                     ? "hidden"
//                     : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
//                 }
//               >
//                 <div className="w-[50%]  flex flex-col gap-y-[30px] ">
//                   <Link to={"/piggybank"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/piggybank_icon_bd14f39497_7ef36812cd/piggybank_icon_bd14f39497_7ef36812cd.png"
//                       title="Piggybank"
//                       subtitle="Dollar Savings"
//                       path="/flexdollar"
//                     />
//                   </Link>

//                   <Link to={"/targets"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/targets_icon_a382a2df4f_10353cd31e/targets_icon_a382a2df4f_10353cd31e.png"
//                       title="Target Savings"
//                       subtitle="Goal-Oriented Savings"
//                       path="/piggybank"
//                     />
//                   </Link>

//                   <Link to={"/flex-dollar"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_dollar_icon_b180813700_8c5227fe52/flex_dollar_icon_b180813700_8c5227fe52.png"
//                       title="Flex Dollar"
//                       subtitle="ollar Savings"
//                       path="/piggybank"
//                     />
//                   </Link>
//                 </div>

//                 <div className="w-[50%]  flex flex-col gap-y-[30px]">
//                   <Link to={"/safelock"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/safelock_icon_7595628576_b010a4fee8/safelock_icon_7595628576_b010a4fee8.png"
//                       title="SafeLock"
//                       subtitle="Fixed Savings"
//                       path="/safelock"
//                     />
//                   </Link>

//                   <Link to={"/flexnaira"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_naira_icon_1_2f3b4c5d6e/flex_naira_icon_1_2f3b4c5d6e.png"
//                       title="Flex Naira"
//                       subtitle="Flexible Wallet"
//                       path="/flexnaira"
//                     />
//                   </Link>

//                   <Link to={"/housemoney"}>
//                     <MenuItem
//                       icon="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/house_money_icon_1_2f3b4c5d6e/house_money_icon_1_2f3b4c5d6e.png"
//                       title="House Money"
//                       subtitle="Home Savings"
//                       path="/housemoney"
//                     />
//                   </Link>
//                 </div>
//               </div>
//             </div>

//             <Link to={"/invest"}>
//               <span>Invest</span>
//             </Link>

//             <span>Stories</span>
//             <span>FAQ</span>

{
  /* <div className="">
              <span
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className="flex items-center gap-x-[5px]"
              >
                Resources <MdKeyboardArrowDown />
              </span>

              <div
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className={
                  !resource
                    ? "hidden"
                    : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
                }
              >
                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">Blog</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">Comics</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">NewsLetter</p>
                    </div>
                  </div>
                </div>

                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">Reports</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">Calculatoe</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-[600] text-[16px]">Ebook</p>
                    </div>
                  </div>
                </div>
              </div>
            </div> */
}

//             <Link to={"/shop"}>
//               <span>Shop</span>
//             </Link>
//           </div>
//         </div>

//         <div className="w-[20%] ">
//           <button>Login</button>
//           <button>Sign up</button>
//         </div>
//       </div>

//       <Nav />
//     </div>
//   );
// };

// export default NavHover;

import React, { useState } from "react";
import { MdKeyboardArrowDown } from "react-icons/md";
import { Link } from "react-router-dom";
import Nav from "./Nav";

const NavHover = () => {
  const [save, setSave] = useState(false);
  const [resource, setResource] = useState(false);

  return (
    <div className="w-full">
      <div className="w-full flex justify-between items-center max-w-[1200px] mx-auto py-5 max-md:hidden">
        <div className="w-[20%]">
          <img
            src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg"
            alt="Logo"
            className="bg-transparent"
          />
        </div>
        <div className="flex justify-between items-center w-[70%] gap-x-[10px] font-[600] text-[16px] text-[#3d4f60]">
          <div
            onMouseEnter={() => setSave(true)}
            onMouseLeave={() => setSave(false)}
            className="relative"
          >
            <span className="flex items-center gap-x-[5px]">
              Save <MdKeyboardArrowDown />
            </span>
            {save && <Nav type="save" />}
          </div>
          <Link to="/invest">
            <span>Invest</span>
          </Link>
          <span>Stories</span>
          <span>FAQ</span>
          <div
            onMouseEnter={() => setResource(true)}
            onMouseLeave={() => setResource(false)}
            className="relative"
          >
            <span className="flex items-center gap-x-[5px]">
              Resources <MdKeyboardArrowDown />
            </span>
            {resource && <Nav type="resource" />}
          </div>
        </div>
      </div>
    </div>
  );
};

export default NavHover;
